...
def nb_reg_given():
    ...
    print("Naive Bayes Score for Give Feedback Classifier Score is ",
    ...
    filename = "save_given_nb_v1"
    save_given_reg = pickle.dump(reg, open(filename, 'wb'))
    '''
    #saved_given_reg = pickle.load(save_given_reg)
    saved_given_reg = pickle.load(open("save_given_svm_v6", 'rb'))
...
def nb_reg_got():
    ...
    print("Naive Bayes Score for Got Feedback Classifier Score is",
          reg.score(df[[constants.UTT, constants.CHATTY_SCORE, constants.SAFETY_SCORE, constants.PUNCTUALITY_SCORE,
                        constants.FRIENDLINESS_SCORE, constants.COMFORTIBILITY_SCORE]], df.got_feedback_classifier_int))
    # print(reg.coef_)
    # save_got_reg = pickle.dumps(reg)
    # saved_got_reg = pickle.loads(save_got_reg)
    print("Model Training Complete")
    filename = "save_got_nb_v1"
